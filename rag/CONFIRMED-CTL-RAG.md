...
| `GET /suggested` | bulk high-confidence ad↔txn suggestions (NOT yet confirmed) — READ-ONLY (#37) | `count`, `min_score`, `suggestions[]` each = CrmAd fields + `suggested_txn{txn_id,source,source_txn_id,txn_date,amount,vendor_name,account_name,payment_ref,memo}` + `score`/`score_pct` + `alt_count`. Params: `min_score` (float, default 0.6, clamp 0..1), `limit` (int, default 200). Scores unconfirmed ads (`list_clearances` minus `ad_confirmations`) via `get_candidate_transactions` (NO Gmail search), best pair per ad ≥ `min_score`, sorted DESC. Same 503/502 CRM contract as `/unconfirmed`. Never writes — mapping still via Map Trx → `/confirm`. |
